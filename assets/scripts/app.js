    ...
    if (taskStorage) {
      ...
      const CleanUpTask = getTask
        .map(function (taskItem, index) {
          ...
          if (isChecked) {
            return `<p id="checkedTask"><input class="checkBOX" data-index =${index} type="checkbox" ${isChecked}> <span class="checkedTaskText">${taskItem.task}</span> <button data-index =${index} class="deleteBtn">      <img src="assets/icons/trash (1).svg" alt="delete icon" />
</button></p>`;
          } else {
            return `<p id="UncheckedTask"><input class="checkBOX" data-index =${index} type="checkbox" ${isChecked}> <span class="UncheckedTaskText">${taskItem.task}</span> <button data-index =${index} class="deleteBtn">      <img src="assets/icons/trash (1).svg" alt="delete icon" />
</button></p>`;
          }
        })
        .join("");
